...
def record(msg_received, header):
    ...
    if viewer_data['db_name'] != 0:
        ...
        if collection.count_documents(query) == 0:
            collection.update_one(query, {'$push': {'views': locator}})
            client.close()
            return {'Message': 'Story view has been recorded.', 'statusCode': 200}

        else:
            client.close()
            return {'Message': 'Story view had been recorded.', 'statusCode': 401}
