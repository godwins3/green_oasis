...
def delete(msg_received, header):
    try:
        ...
        message_id = msg_received['message_id']
    except KeyError:
    ...
    if not str(user_id).isalnum():
        ...
        if receiver_data['user_id'] == 0:
            ...
            try:
                ...
                sender_collection.update_one({"message_id": message_id},
                                             {"$set": {"deleted": 1, "deletedOn": str(datetime.now())}})

                client.close()
                return {"Message": "Message deleted", "statusCode": 200}

            except Exception:
                return {"Message": "Message not deleted", "statusCode": 500}
